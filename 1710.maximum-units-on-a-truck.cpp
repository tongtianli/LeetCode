...
class Solution {
public:
    int onBoardBox(vector<int> &boxType, int truckSize, int &res){
        int num=min(boxType[0], truckSize);
        res += num*boxType[1];
        return truckSize-num;
    }
    int maximumUnits(vector<vector<int>>& boxTypes, int truckSize) {
        ...
        for(int i=0;i<boxTypes.size();i++){
            ...
            truckSize = onBoardBox(boxTypes[i], truckSize, res);
        }
